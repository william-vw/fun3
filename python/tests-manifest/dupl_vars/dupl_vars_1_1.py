...
def query(final_ctu):
    ...
    if Iri('http://example.org/robocop') == Iri('http://example.org/terminator'):
        rule_0(Iri('http://example.org/terminator'), lambda a_0_m: final_ctu())
...
query(lambda: emit(Triple(Iri('http://example.org/robocop'), Iri('http://example.org/part'), Iri('http://example.org/terminator')), {}))