...
def query(x_0, final_ctu):
    ...
    rule_1(x_0, Iri('http://example.org/Canadian'), lambda pe_3_m, ty_4_m: final_ctu(pe_3_m))

def rule_0(p_1, final_ctu):
    data.find(p_1, NS.rdf['type'], Iri('http://example.org/Person'), lambda s, p, o: rule_0_1(s, final_ctu))
    rule_1(p_1, Iri('http://example.org/Person'), lambda pe_3_m, ty_4_m: rule_0_1(pe_3_m, final_ctu))
